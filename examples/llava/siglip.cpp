...
#define KEY_HAS_TEXT_ENC        "siglip.has_text_encoder"
#define KEY_HAS_VIS_ENC         "siglip.has_vision_encoder"
#define KEY_HAS_LLAVA_PROJ      "siglip.has_llava_projector"
#define KEY_HAS_MINICPMV_PROJ   "siglip.has_minicpmv_projector"
#define KEY_MINICPMV_VERSION    "siglip.minicpmv_version"
...
#define KEY_N_POSITIONS         "siglip.text.context_length"
...
#define HEAD_ATT_in_b      "v.head.attention.in_bias"
#define HEAD_ATT_in_w      "v.head.attention.in_weight"
#define HEAD_ATT_out_b     "v.head.attention.out.bias"
#define HEAD_ATT_out_w     "v.head.attention.out.weight"
#define HEAD_ln_b          "v.head.ln.bias"
#define HEAD_ln_w          "v.head.ln.weight"
#define HEAD_ffn_d_b       "v.head.ffn_down.bias"
#define HEAD_ffn_d_w       "v.head.ffn_down.weight"
#define HEAD_ffn_u_b       "v.head.ffn_up.bias"
#define HEAD_ffn_u_w       "v.head.ffn_up.weight"
#define HEAD_probe         "v.head.probe"
...
struct clip_vision_model {
    ...
    // Siglip extra tensors
    struct ggml_tensor * head_attention_b_in;
    struct ggml_tensor * head_attention_w_in;
    struct ggml_tensor * head_attention_b_out;
    struct ggml_tensor * head_attention_w_out;
    struct ggml_tensor * head_ln_b;
    struct ggml_tensor * head_ln_w;
    struct ggml_tensor * head_ffn_d_b;
...
static ggml_cgraph * clip_image_build_graph(clip_ctx * ctx, const clip_image_f32_batch * imgs, struct clip_image_size * load_image_size, bool is_inf = false) {
    ...
    struct ggml_tensor * att = ggml_add(ctx0, ggml_mul_mat(ctx0, model.head_attention_w_in, embeddings), model.head_attention_b_in);
    
    // Assume `tensor` is a tensor with shape [3456, 729, 1, 1]
    int chunk_size = 1152;  // The size of each chunk in the first dimension
    int element_size = ggml_element_size(att);  // Size of one element, typically 4 bytes for f32

    // Create a view for the first chunk (first 1152 entries in the first dimension)
    struct ggml_tensor *Q = ggml_view_4d(ctx0, att, chunk_size, 729, 1, 1, 
                                            att->nb[1], att->nb[2], att->nb[3], 0);

    // Create a view for the second chunk (next 1152 entries in the first dimension)
    struct ggml_tensor *K = ggml_view_4d(ctx0, att, chunk_size, 729, 1, 1, 
                                            att->nb[1], att->nb[2], att->nb[3], 
                                            chunk_size * element_size * att->nb[0]);

    // Create a view for the third chunk (final 1152 entries in the first dimension)
    struct ggml_tensor *V = ggml_view_4d(ctx0, att, chunk_size, 729, 1, 1, 
                                            att->nb[1], att->nb[2], att->nb[3], 
                                            2 * chunk_size * element_size * att->nb[0]);

    struct ggml_tensor * KQ = ggml_mul_mat(ctx0, K, Q);
    KQ = ggml_soft_max_inplace(ctx0, KQ);
    V = ggml_cont(ctx0, ggml_permute(ctx0, V, 1, 0, 2, 3));
    struct ggml_tensor * cur = ggml_mul_mat(ctx0, V, KQ);
    ...
    // Final layernorm
    {
        ...
        cur = ggml_add(ctx0, ggml_mul(ctx0, cur, model.head_ln_w), model.head_ln_b);
    }
    cur = ggml_mul_mat(ctx0, model.head_ffn_d_w, cur);
    cur = ggml_add(ctx0, cur, model.head_ffn_d_b);

    // GELU activation function
    cur = ggml_gelu_inplace(ctx0, cur);

    cur = ggml_mul_mat(ctx0, model.head_ffn_u_w, cur);
...
struct clip_ctx * clip_model_load(const char * fname, const int verbosity = 1) {
    ...
    if (new_clip->has_vision_encoder) {
        ...
        try {
            vision_model.head_attention_w_in  = get_tensor(new_clip->ctx_data, format(HEAD_ATT_in_w, "v", "weight"));
            vision_model.head_attention_b_in   = get_tensor(new_clip->ctx_data, format(HEAD_ATT_in_b, "v", "bias"));

            vision_model.head_attention_w_out  = get_tensor(new_clip->ctx_data, format(HEAD_ATT_out_w, "v", "weight"));
            vision_model.head_attention_w_out   = get_tensor(new_clip->ctx_data, format(HEAD_ATT_out_b, "v", "bias"));

            vision_model.head_ln_w    = get_tensor(new_clip->ctx_data, format(HEAD_ln_w, "v", "weight"));
            vision_model.head_ln_b      = get_tensor(new_clip->ctx_data, format(HEAD_ln_b, "v", "bias"));

            vision_model.head_ffn_d_w  = get_tensor(new_clip->ctx_data, format(HEAD_ffn_d_w, "v", "weight"));
            vision_model.head_ffn_d_b        = get_tensor(new_clip->ctx_data, format(HEAD_ffn_d_b, "v", "bias"));

            vision_model.head_ffn_u_w  = get_tensor(new_clip->ctx_data, format(HEAD_ffn_u_w , "v", "weight"));
            vision_model.head_ffn_u_b    = get_tensor(new_clip->ctx_data, format(HEAD_ffn_u_b , "v", "bias"));
        } catch(const std::exception& e) {
            LOG_TEE("%s: failed to load vision model tensors\n", __func__);
        }
